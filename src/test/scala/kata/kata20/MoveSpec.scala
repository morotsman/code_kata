...
class MoveSpec extends FlatSpec with Matchers {
  ...
  "A Move from a foundationPile to a tableauPile" should "add one card to the tableauPile" in {
    val hidden = true
    val notHidden = false

    var tableauPile = TableauPile(1, Card(Suite.Spades, 2, notHidden))
    var foundationPile = FoundationPile(1,Card(Suite.Spades,1,notHidden))
    var board = BoardBuilder().withTableauPile(0, tableauPile).withFoundationPile(0, foundationPile).build
    var result = board.makeMove(Move(foundationPile, tableauPile, 1))
    result.tableauPiles.head should be(TableauPile(1,Card(Suite.Spades, 1, notHidden),Card(Suite.Spades, 2, notHidden)))
    result.foundationPiles.head should be(FoundationPile(1))
    
    tableauPile = TableauPile(1, Card(Suite.Spades, 2, notHidden), Card(Suite.Hearts,8,hidden))
    foundationPile = FoundationPile(1,Card(Suite.Spades,1,notHidden))
    board = BoardBuilder().withTableauPile(0, tableauPile).withFoundationPile(0, foundationPile).build
    result = board.makeMove(Move(foundationPile, tableauPile, 1))
    result.tableauPiles.head should be(TableauPile(1,Card(Suite.Spades, 1, notHidden),Card(Suite.Spades, 2, notHidden),Card(Suite.Hearts,8,hidden)))
    result.foundationPiles.head should be(FoundationPile(1))     
    
    tableauPile = TableauPile(1, Card(Suite.Spades, 3, notHidden), Card(Suite.Hearts,8,hidden))
    foundationPile = FoundationPile(1,Card(Suite.Spades,2,notHidden),Card(Suite.Spades,1,notHidden))
    board = BoardBuilder().withTableauPile(0, tableauPile).withFoundationPile(0, foundationPile).build
    result = board.makeMove(Move(foundationPile, tableauPile, 1))
    result.tableauPiles.head should be(TableauPile(1,Card(Suite.Spades, 2, notHidden),Card(Suite.Spades, 3, notHidden),Card(Suite.Hearts,8,hidden)))
    result.foundationPiles.head should be(FoundationPile(1,Card(Suite.Spades,1,notHidden)))    
  }
