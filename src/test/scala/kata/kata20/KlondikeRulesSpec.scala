...
class KlondikeRulesSpec extends FlatSpec with Matchers {
  ...
  "If an foundation pile is empty it" should "be possible to move a ace from the tableau or the discard pile" in {
    var stockPile = new StockPile(List(Card(Suite.Hearts, 1, true)))
    ...
    discardPile = new DiscardPile(List(Card(Suite.Clubs, 1, true)))
    ...
    discardPile = new DiscardPile(List(Card(Suite.Clubs, 1, false)))
    board = BoardBuilder().withStockPile(stockPile).withDiscardPile(discardPile).withTableauPiles(emptyTableauPiles).withFoundationPiles(emptyFoundationsPiles).build
    println(board.legalMoves())
    board.legalMoves().length should be(5)

    discardPile = new DiscardPile(List(Card(Suite.Clubs, 2, false)))
    board = BoardBuilder().withStockPile(stockPile).withDiscardPile(discardPile).withTableauPiles(emptyTableauPiles).withFoundationPiles(emptyFoundationsPiles).build
    board.legalMoves().length should be(1)

    discardPile = new DiscardPile(List(Card(Suite.Clubs, 2, false)))
    val foundationPileWithClubAce = new FoundationPile(List(Card(Suite.Clubs, 1, false)))
    board = BoardBuilder().withStockPile(stockPile).withDiscardPile(discardPile).withTableauPiles(emptyTableauPiles).withFoundationPile(0, foundationPileWithClubAce).build
    ...
    discardPile = new DiscardPile(List(Card(Suite.Clubs, 3, false)))
    val foundationPileWithClubTwo = new FoundationPile(List(Card(Suite.Clubs, 2, false),Card(Suite.Clubs, 1, false)))
    ...
    discardPile = new DiscardPile(List(Card(Suite.Hearts, 2, false)))
    ...
    discardPile = new DiscardPile(List(Card(Suite.Clubs, 2, false)))
    val tableauPile = new TableauPile(List(Card(Suite.Diamonds, 2, false)))
    val foundationPileWithDiamondAce = new FoundationPile(List(Card(Suite.Diamonds, 1, false)))
    board = BoardBuilder().withStockPile(stockPile).
      ...
      build
    board.legalMoves().length should be(3)
    board.legalMoves() should be(List(Move(stockPile, discardPile, 1), Move(discardPile, foundationPileWithClubAce, 1),Move(tableauPile, foundationPileWithDiamondAce, 1)))
  }
